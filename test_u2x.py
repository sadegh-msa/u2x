...
class TestU2X(TestCase):
    ...
    async def get_clipboard_value(self):
        async def read_clipboard():
            future = asyncio.Future()

            self.u2x.clipboard.read_text_async(
                None,
                lambda cb, res: future.set_result(cb.read_text_finish(res))
            )

            return future

        loop = asyncio.get_event_loop()
        task = loop.create_task(read_clipboard())
    ...
    def test_unicode_text(self):
        ...
        loop = asyncio.get_event_loop()
        clipboard_value = loop.run_until_complete(self.get_clipboard_value())
        self.assertEqual(clipboard_value.result(), UNICODE_TEXT)
    ...
    def test_xml_text(self):
        ...
        loop = asyncio.get_event_loop()
        clipboard_value = loop.run_until_complete(self.get_clipboard_value())
        self.assertEqual(clipboard_value.result(), XML_TEXT)
